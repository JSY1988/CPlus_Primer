#include "IPLocator.h"
IPLocator::IPLocator(const string &ipdb_name) //打开文件
{
    unsigned char buf[8];
...
IPLocator::~IPLocator() //关闭文件
{
    ipdb.close();
}

string IPLocator::getVersion() //文件版本
{
...
string IPLocator::getIpAddr(const string &ip) //获取地址 以字符串形式
{
    return this->getIpAddr(this->getIpFromString(ip));
}

string IPLocator::getIpAddr(unsigned int ip) //获取地址 以数值形式
{
...
void IPLocator::readFromFile(streamsize offset, unsigned char *buf, int len) //文件读取
{
    ipdb.seekg(offset);
    ipdb.read((char *)buf, len);
}

string IPLocator::readStringFromFile(streamsize offset) //读取字符串从文件
{
    char ch;
    string str;
    ipdb.seekg(offset);
...
unsigned int IPLocator::getIpFromString(const string &ip) //从字符串中获取IP
{
    char *result = NULL;
    unsigned int ret = 0;
...
string IPLocator::getIpString(unsigned int ip) //获取IP字符串
{
    char buf[256];
